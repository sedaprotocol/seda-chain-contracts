...
use crate::{
    ...
    msgs::staking::{test_helpers as staking_test_helpers, StakingConfig},
    TestExecutor,
};
...
pub fn allowlist_works() {
    ...
    let res = staking_test_helpers::reg_and_stake(deps.as_mut(), alice.info(), &alice, None);
    ...
    alice.set_amount(0);
    let _res = staking_test_helpers::unstake(deps.as_mut(), alice.info(), &alice, 100).unwrap();
    let _res = staking_test_helpers::withdraw(deps.as_mut(), alice.info(), &alice, 100).unwrap();
    let res = staking_test_helpers::unregister(deps.as_mut(), alice.info(), &alice);
    assert!(res.is_ok());
