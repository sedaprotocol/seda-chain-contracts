        ...
        for i in 0..DR_PER_BLOCK {
            ...
            let mut dr = DataRequestContract {
                base:    DataRequestBase {
                    ...
                    tally_inputs:       "y"
                        .repeat(INITIAL_DR_CONFIG.tally_input_limit_in_bytes.get() as usize)
                        .as_bytes()
                        .into(),
                    tally_gas_limit:    0,
                    replication_factor: 100,
                    consensus_filter:   "z"
                        .repeat(INITIAL_DR_CONFIG.consensus_filter_limit_in_bytes.get() as usize)
                        .as_bytes()
                        .into(),
                    gas_price:          Uint128::from(0u128),
                    memo:               "m"
                        .repeat(INITIAL_DR_CONFIG.memo_limit_in_bytes.get() as usize)
                        .as_bytes()
                        .into(),
                    payback_address:    "o"
                        .repeat(INITIAL_DR_CONFIG.payback_address_limit_in_bytes.get() as usize)
                        .as_bytes()
                        .into(),
                    seda_payload:       "p"
                        .repeat(INITIAL_DR_CONFIG.seda_payload_limit_in_bytes.get() as usize)
                        .as_bytes()
                        .into(),
            ...
            for j in 0..100 {
                let identity = format!("{:02}69880a0111b4de8da9ccab0225f72babcd0a46de83e0848f028f87a6efb14f", j);
            ...
            //         gas_used: 0,
            //         reveal:
            // "g".repeat(INITIAL_DR_CONFIG.dr_reveal_size_limit_in_bytes.get()
            // as usize).as_bytes().into(),
