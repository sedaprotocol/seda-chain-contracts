    ...
    fn query(self, deps: Deps, _env: Env) -> Result<Binary, ContractError> {
        let binary = match self {
            QueryMsg::GetDataRequest { dr_id } => {
                to_json_binary(&state::may_load_req(deps.storage, &Hash::from_hex_str(&dr_id)?)?)?
            }
            QueryMsg::GetDataRequestCommitment { dr_id, public_key } => {
                let dr = state::load_req(deps.storage, &Hash::from_hex_str(&dr_id)?)?;
            ...
            QueryMsg::GetDataResult { dr_id } => {
                to_json_binary(&state::load_resolved_req(deps.storage, &Hash::from_hex_str(&dr_id)?)?)?
            }
